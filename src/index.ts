...
export default class Peer extends (EventEmitter as new () => PeerEmitter) {
  ...
  readMessage(buffer: Buffer) {
    ...
    if (command === "ping") {
      ...
      this.DEBUG_LOG && console.log(`p2p: verack`);
      ...
      this.DEBUG_LOG && console.warn(`p2p: alert ${payload.toString()}`);
  ...
  connect(options = this.connectOptions) {
    if (!this.promiseConnect) {
      this.promiseConnect = new Promise<void>((resolve, reject) => {
        ...
        this.DEBUG_LOG &&
          console.log(`p2p: Connecting to ${node} on port ${port}`);
        ...
        socket.on("connect", () => {
          this.DEBUG_LOG &&
            console.log(`p2p: Connected to ${node} on port ${port}`);
        ...
        socket.on("error", (error) => {
          this.DEBUG_LOG && console.error(`p2p: Socket error`, error);
        ...
        socket.on("data", (data: Buffer) => {
          // this.DEBUG_LOG && console.log(`p2p: data`, data.length);
          try {
            ...
            this.DEBUG_LOG &&
              console.error(
                `p2p: on data error. Disconnecting. buffer: ${buffer.toString(
                  "hex"
                )}`,
                error
              );
  ...
  disconnect(autoReconnect = false) {
    this.autoReconnect = !!autoReconnect;
    if (this.socket) {
      this.DEBUG_LOG && console.warn(`p2p: Disconnected from ${this.node}`);
      this.socket.destroy();
      this.socket = null;
      this.connected = false;
      this.disconnects++;
